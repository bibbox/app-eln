...
<?php if ($bfa_ata['full_width_header'] == "No") { ?>
	...
	<tr>
		...
		<!-- IARC L.Alteyrac 20160825 -->
		<?php
		$current_url = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
	
		if (preg_match("#category#", $current_url)) {
			// image file for Table of Contents
			$file = "TOC_IARC.jpg";
			$header_text = single_cat_title('',false) . " - Table of Contents";
			$header_text_color = 'iarc-header-light';
		}
		else if (preg_match("#/?s=#", $current_url)) {
			// IARC L.Alteyrac 20150902: image file for Search results
			$file = "SearchResults_IARC.jpg";
			$header_text = $wp_query->found_posts . " result(s) for \"" . get_search_query() . "\"";
			$header_text_color = 'iarc-header-dark';
		}
		else {
			// image file for ELN
			$file = "Header_IARC.jpg";
			$header_text = '';
			$header_text_color = '';
		}
		$image = get_stylesheet_directory_uri() . '/images/header/' . $file ;
		?>
		
		<div class="iarc-header" style="<?php echo "background-image: url(" . $image . ")" ; ?>" >
			<p class="<?php echo $header_text_color; ?>"><?php echo $header_text; ?></p>
		</div>
